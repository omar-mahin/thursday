...
/**
 * The floating panel's frame.
 *
 * Only the chrome around the iframe: the bar to drag it by, the corner to
 * resize from, and the border that makes it read as a window over the page
 * rather than part of it. The panel's own appearance lives in the panel.
 */
...
.pf-root {
  ...
  border-radius: 12px;
  background: var(--thu-bg);
  /* Heavier than the toolbar's: this is a window, and it has to separate from
     whatever design is behind it. */
...
.pf-bar {
  ...
  height: 36px;
  padding: 0 8px;
  border-bottom: 1px solid var(--thu-line);
  background: var(--thu-bg);
  cursor: grab;
  /* The bar is a handle, so a drag across it must not start a text selection. */
  user-select: none;
}
.pf-root[data-dragging="true"] .pf-bar { cursor: grabbing; }
.pf-grip {
  width: 10px;
  height: 16px;
  flex: none;
  background-image: radial-gradient(circle at 2px 2px, var(--thu-fg-dim) 1.4px, transparent 1.6px);
...
.pf-btn:hover { background: var(--thu-bg-hover); }
.pf-btn:focus-visible { outline: 2px solid var(--thu-accent); outline-offset: 2px; }
...
.pf-resize {
  ...
  height: 18px;
  cursor: nwse-resize;
  /* Two strokes, so the corner reads as a grip without a background. */
  background-image: linear-gradient(
    135deg,
    transparent 0 40%,
    var(--thu-fg-dim) 40% 50%,
    transparent 50% 60%,
    var(--thu-fg-dim) 60% 70%,
    transparent 70%
  );
  opacity: 0.7;
}
